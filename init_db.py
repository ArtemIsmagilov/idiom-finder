...
def create_xdxf_table():
    ...
    session = db.get_conn()
    session.cur.executescript(
        open("database/create_xdxf_idoms.sql", encoding="utf-8").read()
    )
    session.cur.executemany(
        "INSERT INTO xdxf_idioms(name) VALUES (?);",
        [[child.text] for child in root.iter("k")],
    )
    session.conn.commit()
...
def create_wiki_table():
    ...
    async def insert_idiom(idiom):
        async with conn.execute(
            "INSERT INTO idiom(name) VALUES(?) RETURNING *;", (idiom,)
        ) as cur:
            return await cur.fetchone()

    async def insert_form_idiom(form_idiom, idiom_id):
        async with conn.execute(
            "INSERT INTO form_idiom(name, idiom_id) VALUES(?, ?) RETURNING *;",
            (form_idiom, idiom_id),
        ) as cur:
            return await cur.fetchone()
    ...
    async def main():
        ...
        async with aiohttp.ClientSession() as client, aiosqlite.connect(
            "database.sqlite3"
        ) as conn:
            conn.row_factory = aiosqlite.Row

            await conn.executescript(
                open("database/init_db.sql", encoding="utf-8").read()
            )
            ...
            while next_page:
                ...
                next_body = soup.find(
                    "a", title="Category:English idioms", string=re.compile("next page")
                )
                if next_body:
                ...
                for element in page_idioms:
                    async with client.get(
                        join_strings(domain, element.a.get("href"))
                    ) as sub_response:
                        sub_html = await sub_response.text()

                    sub_soup = BeautifulSoup(sub_html, "lxml")
                    another_words = sub_soup.find_all(
                        "b", class_=re.compile("Latn form-of lang-en")
                    )
            ...
            await conn.executescript(
                open("database/create_wiki_idioms.sql", encoding="utf-8").read()
            )
            await conn.commit()
...
def create_all_idioms():
    session = db.get_conn()
    session.cur.executescript(
        open("database/create_all_idoms.sql", encoding="utf-8").read()
    )
    session.conn.commit()
    db.close_conn(session)
...
def extend_pronouns():
    with (
        open("database/pronouns.txt", encoding="utf-8") as pr_file,
