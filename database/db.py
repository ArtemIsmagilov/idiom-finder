...
def update(cur: Cursor, from_name: str, to_name: str) -> Row | None:
    cur.execute(
        "UPDATE all_idioms SET name=:to_name WHERE name=:from_name;",
...
def find(cur: Cursor, text: str) -> list[Row | None]:
    data = tuple(
        {"sentence": re.sub(r" {2,}", " ", l.strip())}
        for l in re.split(r"[?.!]+", re.sub(r"\n| {2,}|\.{2,}", " ", text))
        if l.strip()
    )
...
def like(cur: Cursor, text: str) -> list[Row | None]:
    cur.execute(
        "SELECT name FROM all_idioms WHERE name LIKE :name ORDER BY name;",
        {"name": text},
    )
    return cur.fetchall()
